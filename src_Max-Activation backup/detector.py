...
class WatermarkDetector:
    ...
    def detect(
        ...
        text: str,
        secret_key: str,
        prompt_len: int = 0, # <--- Thêm tham số này
        ...
        with torch.no_grad():
            ...
            all_hidden_states = outputs.hidden_states[sae_cfg.target_layer][0] # [N, d_model]

        # --- Step 2: Reconstruct delta_h and compute scores ---
        from orthogonal import compute_orthogonal_steering_vector
        import hashlib # Thêm thư viện này nếu ở đầu file chưa có
        
        scores = []
        valid_tokens_count = 0  # Biến đếm số token thực sự được đo
        start_idx = max(prompt_len, wm_cfg.context_window) # Vẫn giữ để bỏ qua phần prompt
        ...
        for t in range(start_idx, N - 1):
            ...
            from prf import get_max_activation_seed
            
            # --- Tính Toán Target Feature (Max-Activation Index) ---
            current_hidden_state = all_hidden_states[t]
            seed = get_max_activation_seed(current_hidden_state, secret_key)
            ...
            # --- Tính Toán OLS trực tiếp với v_target ở trên ---
            # (Đã xóa hoàn toàn LSH và context_window ở đây)
