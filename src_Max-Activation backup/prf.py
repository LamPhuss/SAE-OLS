...
def get_max_activation_seed(hidden_state: torch.Tensor, secret_key: str) -> int:
    """
    Trích xuất đặc trưng ngữ nghĩa dựa trên nơ-ron có độ kích hoạt cao nhất (Max-Activation).
    Miễn nhiễm với sai số dấu phẩy động và ổn định trước từ đồng nghĩa.
    
    Args:
        hidden_state: Tensor chứa hidden state hiện tại (shape: [..., d_model])
        secret_key: Khóa bảo mật của watermark
        
    Returns:
        Seed (Integer) ổn định cho bộ sinh số ngẫu nhiên.
    """
    # 1. Tìm index của nơ-ron sáng nhất (argmax)
    max_index = torch.argmax(hidden_state).item()
    
    # 2. Băm kết hợp Secret Key và Max Index
    # Dùng hashlib để tạo hiệu ứng tuyết lở (avalanche effect), đảm bảo 
    # các max_index khác nhau sẽ sinh ra các seed hoàn toàn khác biệt.
    combo_string = f"{secret_key}_{max_index}"
